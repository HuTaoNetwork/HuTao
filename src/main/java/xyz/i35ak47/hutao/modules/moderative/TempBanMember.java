...
public class TempBanMember extends Command {

    /*
     * Command "tban" based on https://github.com/HuTaoNetwork/HuTao/blob/staging/src/main/java/xyz/i35ak47/hutao/modules/moderative/BanMember.java
     */

    @SuppressWarnings("SpellCheckingInspection")
    public TempBanMember() {
        super("tban");
    }
        ...
        if (!event.isFromType(ChannelType.PRIVATE)) {
            if (event.getGuild().getSelfMember().hasPermission(Permission.BAN_MEMBERS)) {
                if (Objects.requireNonNull(event.getMember()).hasPermission(Permission.BAN_MEMBERS)
                    ...
                    if (msgComparableRaw.length >= 3 && msgComparableRaw[2].startsWith("<") && !msgComparableRaw[1].isEmpty()) {
                        if (event.getMessage().getMentionedMembers().get(0).getId().equals(event.getAuthor().getId())) {
                            ...
                            String reason = "";
                            for (int i = 3; i < msgComparableRaw.length; i++) {
                            ...
                            event.getGuild().ban(user, Integer.parseInt(msgComparableRaw[1]), reason).queue(
                            response -> {
                                /*
                                 * Send ban message
                                 */
